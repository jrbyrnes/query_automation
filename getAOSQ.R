getAOSQ <- function(clauses,tablename){
  ...
  if (nchar(clauses[9]) == 0){
    ...
    weight <- paste("AVG(",clauses[9],") pw", sep = " ")
  }
  
  i_select_clause = paste('SELECT', clauses[6], ', SUM(', clauses[2], ') srev, ', weight, sep = " ")
  ...
  i_group_clause = paste('GROUP BY', clauses[6], ", orderid", sep = " ")
  
  i_clause = paste(i_select_clause, i_from_clause, i_where_clause, i_group_clause, sep = " ")
  
  o_select_clause = paste("SELECT CONVERT(FLOAT, SUM(t.srev)) / CONVERT(FLOAT, SUM(t.pw)),", out_group, sep = " ")
